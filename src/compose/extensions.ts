...
export function compareExtensions(
	...
	for (const name of Object.keys(targetByName)) {
		...
		if (!curr) {
			toAdd.push(svc);
		} else if (curr.image !== svc.image) {
			// Image changed - needs update
			toUpdate.push(svc);
		}
		// If image is same, no action needed
	}
...
export async function handleOverlayExtensions(
	...
	// Call update-hostapp-extensions with the target image list
	// The script handles both adding new and removing old extensions
	try {
		// Use execFile with argument array to avoid shell injection
		const args = targetImages.length > 0 ? ['-t', ...targetImages] : [];
		await execFile('update-hostapp-extensions', args);
	} catch (err) {
