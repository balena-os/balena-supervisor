...
export const isInitialized = async (configJson: ConfigJsonConfigBackend) => {
	...
	try {
		...
		// so log error and skip initialization.
		log.error(
			'Error reading config.json, skipping extra firmware initialization',
		);
		return null;
	}
	...
	try {
		...
		// there was some other error getting the volume, skip
		// initialization so we don't get stuck in an initialization loop.
		if (!isNotFoundError(e)) {
			log.error(`Error getting extra firmware volume: ${(e as Error).message}`);
			return null;
		}
	}
...
export const initialize = async (configJson: ConfigJsonConfigBackend) => {
