...
do ->
	...
	exports.stopContainer = (req, res) ->
		...
		knex('app').select()
		.then (apps) ->
			throw new Error('Cannot stop an app container') if _.any(apps, (app) -> req.params.id == app.containerId)
			container.inspectAsync()
		.then (cont) ->
			throw new Error('Cannot stop supervisor container') if cont.Name == '/resin_supervisor'
	...
	exports.deleteContainer = (req, res) ->
		...
		nex('app').select()
		.then (apps) ->
			throw new Error('Cannot remove an app container') if _.any(apps, (app) -> req.params.id == app.containerId)
			container.inspectAsync()
		.then (cont) ->
			throw new Error('Cannot remove supervisor container') if cont.Name == '/resin_supervisor'
