...
describe 'config.coffee', ->
	...
	it 'initializes correctly', ->
		expect(config.init).to.be.fulfilled

	it 'reads and exposes values from the config.json', ->
		promise = config.init.then ->
			config.getMany(['apiEndpoint', 'applicationId'])
	...
	it 'generates a uuid and stores it in config.json', ->
		promise = config.init.then ->
			config.get('uuid')
		promise2 = config.init.then ->
			fs.readFileAsync('./test/data/config.json')
			.then(JSON.parse).get('uuid')
		Promise.all([
	...
	it 'does not allow setting an immutable field', ->
		promise = config.init.then ->
			config.set({ username: 'somebody else' })
