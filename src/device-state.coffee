...
module.exports = class DeviceState extends EventEmitter
	constructor: ({ @db, @config, @eventTracker }) ->
		...
		@applications = new ApplicationManager({ @config, @logger, @db, @reportCurrentState, @eventTracker })
		@on 'error', (err) ->
		...
		.then =>
			...
			Promise.map tryParseArray(apps), (app) =>
				...
				.then (image) =>
					...
					@applications.images.get(appAsState.image)
					.then =>
						@applications.images.markAsSupervised(appAsState.image)
					.then =>
	...
	init: ->
		@config.getMany([ 'logsChannelSecret', 'pubnub', 'offlineMode', 'loggingEnabled', 'initialConfigSaved' ])
		.then (conf) =>
			@logger.init({
				pubnub: conf.pubnub
	...
	setTarget: (target) ->
		...
		.then =>
			Promise.using @writeLockTarget(), =>
				...
				@db.transaction (trx) =>
					Promise.try =>
						@config.set({ name: target.local.name, targetStateSet: 'true' }, trx) if target.local?.name?
					.then =>
