...
describe('config/power-fan', () => {
	...
	const generateConfigJsonBackend = () =>
		new ConfigJsonConfigBackend(schema, PROTECTED_FIELDS);
	...
	it('handles setting configs correctly when target configs are empty string BBB', async () => {
		await testfs({
			[CONFIG_PATH]: stripIndent`
			{
				"os": {
	...
	it('does not touch unmanaged fields in config.json when setting boot configs', async () => {
		await testfs({
			[CONFIG_PATH]: stripIndent`
			{   
				"apiEndpoint": "https://api.balena-cloud.com",
		...
		// Sanity check that config.json is unchanged
		const configJson = await hostUtils.readFromBoot(CONFIG_PATH, 'utf-8');
		expect(configJson).to.deep.equal(
			JSON.stringify({
				apiEndpoint: 'https://api.balena-cloud.com',
	...
	it('does not touch unmanaged fields in config.json when removing boot configs', async () => {
		await testfs({
			[CONFIG_PATH]: stripIndent`
			{   
				"apiEndpoint": "https://api.balena-cloud.com",
