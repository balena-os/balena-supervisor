...
export class PowerFanConfig extends ConfigBackend {
	...
	public async setBootConfig(opts: ConfigOptions): Promise<void> {
		...
		// currentConf is a reference to ConfigJsonConfigBackend's cache,
		// so we don't want to mutate it directly. This is because
		// ConfigJsonConfigBackend.set will check equality against
		// its cache when determining if it needs to write to config.json.
		// If we mutate the cache, the check will never detect a change
		// and won't write to config.json.
