...
gosuper: go-builder
	...
	docker run --name resin_build_gosuper_$(JOB_NAME) -v $(shell pwd)/gosuper/bin:/usr/src/app/bin -e USER_ID=$(shell id -u) -e GROUP_ID=$(shell id -g) -e GOARCH=$(GOARCH) GOARM=$(GOARM) resin/go-supervisor-builder:$(SUPERVISOR_VERSION)
