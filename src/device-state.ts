...
const currentVolatile: DeviceReportFields = {};
const writeLock = updateLock.writeLock;
const readLock = updateLock.readLock;
const cancelDelay: null | (() => void) = null;
let maxPollTime: number;
const intermediateTarget: TargetState | null = null;
let applyBlocker: Nullable<Promise<void>>;

export let lastSuccessfulUpdate: number | null = null;
export let failedUpdates: number = 0;
export let applyInProgress = false;
export let applyCancelled = false;
export let lastApplyStart = process.hrtime();
export let scheduledApply: { force?: boolean; delay?: number } | null = null;
...
export const initialized = (async () => {
	...
	await triggerApplyTarget({ initial: true });
})();
