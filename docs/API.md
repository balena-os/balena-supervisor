...
#### Request body
Can be a JSON object with `force` and `cancel` properties. If `force` is true, update locks will be overridden by the Supervisor. If `cancel` is true, the Supervisor will abort its current state apply operation to apply the most up-to-date target state. This is useful for bailing out of an image pull that's hanging in order to retry it, for example.
```json
{
	"force": true,
	"cancel": true
