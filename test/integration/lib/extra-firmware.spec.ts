...
describe('lib/extra-firmware', () => {
	...
	describe('isInitialized', () => {
		...
		it('should return null (skip initialization) and log error if config.json is corrupted or an unexpected format', async () => {
			tfs = await testfs({
			...
			expect(await extraFirmware.isInitialized(configJsonBackend)).to.be.null;
		...
		it('should return null (skip initialization) and log error if error getting extra firmware volume', async () => {
			tfs = await testfs({
				[CONFIG_PATH]: JSON.stringify({
					apiEndpoint: 'foo',
					deviceId: 123,
