...
module.exports = ({ db, configPath }) ->
	...
	config.constants = constants = require './constants'
	...
	funcs =
		...
		offlineMode: ->
			config.getMany([ 'resinApiEndpoint', 'supervisorOfflineMode' ])
			.spread (apiEndpoint, supervisorOfflineMode) ->
				return Boolean(supervisorOfflineMode) or !Boolean(apiEndpoint)
		pubnub: ->
			config.getMany(['pubnubSubscribeKey', 'pubnubPublishKey' ])
	...
	config.configJsonPath = configJsonPath = Promise.method ->
	...
	generateRequiredFields = ->
		config.getMany([ 'uuid', 'deviceApiKey', 'apiSecret', 'logsChannelSecret' ])
		.spread (uuid, deviceApiKey, apiSecret, logsChannelSecret) ->
			if !uuid? or !deviceApiKey? or !apiSecret? or !logsChannelSecret?
				...
				config.set({ uuid, deviceApiKey, apiSecret, logsChannelSecret })
	...
	config.getMany = (keys) ->
		...
		Promise.all(_.map(keys, config.get))
	...
	config.set = Promise.method (keyValues) ->
