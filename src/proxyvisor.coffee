...
exports.sendUpdates = ->
	knex('dependentApp').select()
	.map (app) ->
		dockerUtils.getImageEnv(app.imageId)
		.then (imageEnv) ->
			return imageEnv.RESIN_DEPENDENT_DEVICES_HOOK_ADDRESS if imageEnv.RESIN_DEPENDENT_DEVICES_HOOK_ADDRESS?
			return app.config.RESIN_DEPENDENT_DEVICES_HOOK_ADDRESS if app.config.RESIN_DEPENDENT_DEVICES_HOOK_ADDRESS?
			return "#{config.proxyvisorHookReceiver}/v1/devices/"
		.then (endpoint) ->
			return {
				appId: app.appId
				endpoint
